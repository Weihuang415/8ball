...
def downloadVid(playlist_id):
    ...
    choose_video_id = video_ids[index]
...
def play_video_process(pickedVid):    
    if os.path.exists(pickedVid):
        cvlc_cmd = f"cvlc {os.path.abspath(pickedVid)} --fullscreen --aspect-ratio=1.777 --repeat --extraintf=http --http-host=127.0.0.1 --http-port=8080 --http-password=asdf"
        os.system(cvlc_cmd)
...
def playNext():
    ...
    if cachedVids:
        ...
        # Send HTTP request to VLC to play the next video
        # play_url = f'http://127.0.0.1:8080/requests/status.json?command=in_play&input={os.path.abspath(pickedVid)}' 
        # headers = {'Authorization' : basic_auth("", "asdf")}
