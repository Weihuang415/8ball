...
'''
(v) After downloading, it removes the 'same file name video'
(v) [00, 01, 02] > [00, 01, 02]
...
def downloadVid(playlist_id):
    ...
    #keep the filename from yt_01 ~ yt_05.mp4 
...
def Transition(new_video):
    if os.path.exists(new_video):
        ffmpeg_cmd = f'ffmpeg -i {new_video} -vf "fade=t=out:st=4.06:d=3" -c:a copy fade.mp4'
        os.system(ffmpeg_cmd)
        print("ffmpeg_transition", new_video)
...
def playVideo():
    ...
    if cachedVids:
        ...
        pickedVid = cachedVids[index % Folder_LimitVid - 1]
...
def playNext():
    ...
    if cachedVids:
        ...
        if response.status_code == 200:
            print(f'VLC is playing the default video: {defaultVid}')
        else:
        ...
        if response.status_code == 200:
            print(f'VLC is playing the picked video: {pickedVid}')
        else:
