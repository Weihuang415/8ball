...
def Transition(pickedVid):
    if os.path.exists(pickedVid):
        ffmpeg_cmd = f'ffmpeg -i {os.path.abspath(pickedVid)} -vf "fade=t=out:st=4.06:d=3" -c:a {os.path.abspath(pickedVid)}'

        os.system(ffmpeg_cmd)
        print("ffmpeg_transition", pickedVid)
